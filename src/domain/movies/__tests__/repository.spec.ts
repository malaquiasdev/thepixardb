...
import { findMoviesBy } from '../repository';

let spy = null;

describe('# Movies - repository', () => {
  beforeEach(() => {
    spy = jest.spyOn(dynamoDB, 'executePartiQL').mockResolvedValue([
      {
  ...
  test('## findMoviesBy - should be executed once without genres and return MovieModel as array', async () => {
    const response = await findMoviesBy('movies', 'en');
    expect(spy).toHaveBeenCalledWith(`SELECT * FROM movies WHERE "language" = 'EN' `);
    expect(response).toMatchSnapshot();
  });

  test('## findMoviesBy - should be executed once with genres and return MovieModel as array', async () => {
    const response = await findMoviesBy('movies', 'en', 'action');
    expect(spy).toHaveBeenCalledWith(`SELECT * FROM movies WHERE "language" = 'EN'  AND CONTAINS("genres", 'ACTION')`);
    expect(response).toMatchSnapshot();
  });
});
